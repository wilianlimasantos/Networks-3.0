...
public class NetworkQuantumStorage extends SlimefunItem implements DistinctiveItem {
    ...
    private static final ItemStack BACK_INPUT = new CustomItemStack(
            ...
            Theme.PASSIVE + "Entrada");

    private static final ItemStack BACK_ITEM = new CustomItemStack(
            Material.BLUE_STAINED_GLASS_PANE,
            Theme.PASSIVE + "Item Armazenado");

    private static final ItemStack NO_ITEM = new CustomItemStack(
            Material.RED_STAINED_GLASS_PANE,
            Theme.ERROR + "Nenhum Item Registrado",
            Theme.PASSIVE + "Clique no ícone abaixo enquanto",
            Theme.PASSIVE + "segura um item para registrá-lo.");

    private static final ItemStack SET_ITEM = new CustomItemStack(
            Material.LIME_STAINED_GLASS_PANE,
            Theme.SUCCESS + "Definir Item",
            Theme.PASSIVE + "Arraste um item sobre este painel para registrá-lo.",
            Theme.PASSIVE + "Shift Clique para alterar descarte");
    ...
    private static final Map<Location, QuantumCache> CACHES = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    private void onTick(Block block) {
        ...
        // Re-absorb any items currently in the output slot back into the cache
        // This ensures the cache amount is always authoritative and prevents
        // double-counting
        final ItemStack existingOutput = blockMenu.getItemInSlot(OUTPUT_SLOT);
        if (existingOutput != null && existingOutput.getType() != Material.AIR
                && cache.getItemStack() != null && StackUtils.itemsMatch(cache, existingOutput, true)) {
            int leftover = cache.increaseAmount(existingOutput.getAmount());
            existingOutput.setAmount(leftover);
        }
        ...
        // Always sync the cache to BlockStorage to prevent stale data on restart
    ...
    private void setItem(@Nonnull BlockMenu blockMenu, @Nonnull Player player) {
        ...
        if (cache == null || cache.getAmount() > 0) {
            player.sendMessage(
                    Theme.WARNING + "O Armazenamento Quântico deve estar vazio antes de alterar o item definido.");
            return;
        }
    ...
    public static ItemStack getItemStack(@Nonnull QuantumCache cache, @Nonnull BlockMenu blockMenu, int amount) {
        // Withdraw exclusively from cache - onTick handles output slot population
        // This prevents duplication caused by withdrawing from both cache and output
        // slot
        ItemStack withdrawn = cache.withdrawItem(amount);
        syncBlock(blockMenu.getLocation(), cache);
        return withdrawn;
    }
    ...
    public static void updateDisplayItem(@Nonnull BlockMenu menu, @Nonnull QuantumCache cache) {
        if (cache.getItemStack() == null) {
            ...
            lore.add("");
            lore.add(Theme.CLICK_INFO + "Descarte: " + Theme.PASSIVE
                    + StringUtils.toTitleCase(String.valueOf(cache.isVoidExcess())));
            lore.add(Theme.CLICK_INFO + "Quantidade: " + Theme.PASSIVE + cache.getAmount());
