...
public abstract class NetworkObject extends SlimefunItem implements AdminDebuggable {
    ...
    protected void prePlace(@Nonnull PlayerRightClickEvent event) {
        ...
        if (blockOptional.isPresent()) {
            ...
            for (BlockFace checkFace : CHECK_FACES) {
                ...
                // Even if the node definition doesn't have a node yet (between ticks),
                // check which network(s) claim this location via the NETWORKS map.
                // This prevents placing bridges/nodes that would connect two separate networks.
                Location foundController = findOwningController(checkBlock.getLocation());
                if (foundController != null) {
                    if (controllerLocation == null) {
                        controllerLocation = foundController;
                    } else if (!controllerLocation.equals(foundController)) {
                        cancelPlace(event);
                        return;
                    }
                }
    ...
    /**
     * Finds which controller owns a given location by checking all active networks.
     * Returns the controller location if found, null otherwise.
     */
    @Nullable
    private static Location findOwningController(@Nonnull Location location) {
        for (Map.Entry<Location, NetworkRoot> entry : NetworkController.getNetworks().entrySet()) {
            if (entry.getValue().getNodeLocations().contains(location)) {
                return entry.getKey();
            }
        }
        return null;
    ...
    protected void cancelPlace(PlayerRightClickEvent event) {
        event.getPlayer().sendMessage(Theme.ERROR.getColor() + "Esta colocação conectaria dois controladores!");
        event.cancel();
    }
