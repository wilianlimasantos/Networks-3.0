    ...
    public NetworkCard(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, int size) {
        ...
        addItemHandler(new ItemUseHandler() {
            ...
            public void onRightClick(PlayerRightClickEvent e) {
                ...
                e.cancel();
                if (card.getAmount() > 1) {
                    player.sendMessage(Theme.WARNING + "Separe os cartões antes de atribuir um item.");
                    return;
                }

                if (isBlacklisted(stackToSet)) {
                    player.sendMessage(Theme.WARNING + "Este tipo de item não pode ser armazenado em um Cartão de Rede.");
                    return;
                }
                ...
                if (cardItem instanceof NetworkCard networkCard) {
                    ...
                    if (cardInstance.getAmount() > 0) {
                        e.getPlayer().sendMessage(Theme.WARNING + "Um cartão deve estar vazio antes de tentar atribuir um item.");
                        return;
                    }
