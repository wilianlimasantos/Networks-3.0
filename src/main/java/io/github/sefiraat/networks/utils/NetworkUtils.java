...
public class NetworkUtils {
    ...
    public static void applyConfig(@Nonnull NetworkDirectional directional, @Nonnull ItemStack itemStack, @Nonnull BlockMenu blockMenu, @Nonnull Player player) {
        ...
        if (string == null) {
            player.sendMessage(Theme.ERROR + "Direção: " + Theme.PASSIVE + "Não definida");
            return;
        }

        directional.setDirection(blockMenu, BlockFace.valueOf(string));
        player.sendMessage(Theme.ERROR + "Direção: " + Theme.PASSIVE + "Aplicada com sucesso");
                ...
                if (templateStack != null && templateStack.getType() != Material.AIR) {
                    ...
                    for (ItemStack stack : player.getInventory()) {
                        if (StackUtils.itemsMatch(stack, templateStack)) {
                            ...
                            player.sendMessage(Theme.SUCCESS + "Item [" + i + "]: " + Theme.PASSIVE + "Item adicionado ao filtro");
                            worked = true;
                            break;
                        }
                    }
                    if (!worked) {
                        player.sendMessage(Theme.WARNING + "Item [" + i + "]: " + Theme.PASSIVE + "Itens insuficientes para preencher o filtro");
                    }
                } else if (directional instanceof NetworkPusher) {
                    player.sendMessage(Theme.WARNING + "Item [" + i + "]: " + Theme.PASSIVE + "Nenhum item na configuração armazenada");
                }
                i++;
            }
        } else {
            player.sendMessage(Theme.WARNING + "Itens: " + Theme.PASSIVE + "Nenhum item na configuração armazenada");
        }
    }
