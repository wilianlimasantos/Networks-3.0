...
public class NetworkNode {
    ...
    private void killAdditionalController(@Nonnull Location location) {
        ...
        if (toDrop != null) {
            // Remove the killed controller's network from all registries FIRST
            // to prevent it from being used by any concurrent operations
            NetworkRoot killedRoot = NetworkController.getNetworks().remove(location);
            if (killedRoot != null) {
                // Clean up claimed locations for the killed controller's network
                NetworkController.getClaimedLocations().removeAll(killedRoot.getNodeLocations());
            }
            NetworkStorage.getAllNetworkObjects().remove(location);

            // Drop the item and set block to AIR
            try {
                ...
                // Folia doesn't support the global scheduler, run inline
                location.getWorld().dropItemNaturally(location, toDrop);
                block.setType(Material.AIR);
            }
        }
